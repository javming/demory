...
public class Randoms {
    public static void main(String[] args) {
        ...
        char[] charArray = str.substring(j,i).toCharArray();//将取到的7位数做成数组 		//将26位字母做成数组
        String [] arr = {"A", "B", "C", "D","E", "F"}; 		//将字母数组随机取3个字母组成一个字符串，一共组成3个字符串放到目标数组target中
        ...
        while (zimucount<9||timecount<7) {//开始组合
            if(insertflag){//默认为ture，先加字母，你也可以先加数字
                if (zimucount<9) {//如果uniqueId插入的字幕总数没超过9个
                    uniqueId.append(arr[random.nextInt(6)]);//则在字母数组中随机选一个插入
                    zimucount++;//对应加1
